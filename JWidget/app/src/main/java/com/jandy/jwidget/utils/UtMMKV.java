...
public final class UtMMKV {
    ...
    private static final Map<String, UtMMKV> MMKV_UTILS_MAP = new HashMap<>();
    private MMKV mmkv;

    public static UtMMKV getInstance() {
     ...
     * @return
     */
    public static UtMMKV getInstance(int mode, String cryptKey) {
        return getInstance("", mode, cryptKey);
    }


    public static UtMMKV getInstance(String name) {
        return getInstance(name, MMKV.SINGLE_PROCESS_MODE, null);
    }

    public static UtMMKV getInstance(String name, int mode, String cryptKey) {
        return getInstance(name, mode, cryptKey, "");
    }

    public static UtMMKV getInstance(String name, String rootPath, int mode) {
        return getInstance(name, mode, "", rootPath);
    }

    public static UtMMKV getInstance(String name, int mode, String cryptKey, String rootPath) {
        if (isSpace(name)) name = MMKV_KEY_DEF;
        UtMMKV mmkvUtils = MMKV_UTILS_MAP.get(name);
        if (mmkvUtils == null) {
            synchronized (UtMMKV.class) {
                mmkvUtils = MMKV_UTILS_MAP.get(name);
                if (mmkvUtils == null) {
                    mmkvUtils = new UtMMKV(name, mode, cryptKey, rootPath);
    ...
    private UtMMKV(String name, int mode, String cryptKey, String rootPath) {
